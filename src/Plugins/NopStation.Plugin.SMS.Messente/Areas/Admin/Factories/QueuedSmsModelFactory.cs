    ...
    public class QueuedSmsModelFactory : IQueuedSmsModelFactory
    {
            ...
            QueuedSms queuedSms, bool excludeProperties = false)
        {
            if (queuedSms != null)
            {
                ...
                model ??= queuedSms.ToModel<QueuedSmsModel>();
                ...
                {
                    model.CustomerName = await _localizationService.GetResourceAsync("Admin.NopStation.Messente.QueuedSmss.All");
                }
                else
                {
                    ...
                    {
                        model.CustomerId = 0;
                        model.CustomerName = await _localizationService.GetResourceAsync("Admin.NopStation.Messente.QueuedSmss.Unknown");
                    }
