...
namespace NopStation.Plugin.SMS.Messente.Services
{
    public partial class WorkflowSmsService : IWorkflowNotificationService
    {
        ...
        private readonly MessenteSmsSettings _bulkSmsSettings;
        ...
        public WorkflowSmsService(ICustomerService customerService,
            ...
            MessenteSmsSettings bulkSmsSettings)
        {
        ...
        protected string GetFormattedPhoneNumber(string phoneNumber)
        {
            ...
            if (_bulkSmsSettings.CheckPhoneNumberRegex)
            {
                var match = Regex.Match(phoneNumber, _bulkSmsSettings.PhoneNumberRegex, RegexOptions.IgnoreCase);
            ...
            if (_bulkSmsSettings.CheckIntlDialCode)
            {
                if (!phoneNumber.StartsWith(_bulkSmsSettings.IntlDialCode))
                {
                    if (_bulkSmsSettings.RemoveFirstNDigitsWhenLocalNumber > 0)
                        phoneNumber = phoneNumber[_bulkSmsSettings.RemoveFirstNDigitsWhenLocalNumber..];

                    phoneNumber = _bulkSmsSettings.IntlDialCode + phoneNumber;
                }
            }
