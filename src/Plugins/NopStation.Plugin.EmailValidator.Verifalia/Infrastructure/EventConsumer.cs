...
namespace NopStation.Plugin.EmailValidator.Verifalia.Infrastructure
{
        ...
        IConsumer<ModelPreparedEvent<BaseNopModel>>
    {
        ...
        public EventConsumer(IVerifaliaEmailService verifaliaEmailService,
        ...
        protected async Task<bool> ValidateEmailAsync(string email)
        {
            ...
            if (_verifaliaEmailValidatorSettings.BlockedDomains.Contains(domain))
                return false;

            var verifaliaEmail = await _verifaliaEmailService.GetVerifaliaEmailByEmailAsync(email);
            if (verifaliaEmail != null)
            {
                if (verifaliaEmail.IsValid(_verifaliaEmailValidatorSettings.AllowRiskyEmails))
                    return true;

                if (verifaliaEmail.UpdatedOnUtc.AddHours(_verifaliaEmailValidatorSettings.RevalidateInvalidEmailsAfterHours) > DateTime.UtcNow)
                    return false;
            }

            var validation = await _validationService.ValidationEmailAsync(email);
            var validationEntry = validation.Entries[0];

            await validationEntry.SaveAsync(_verifaliaEmailService, verifaliaEmail);
            await _staticCacheManager.RemoveAsync(CacheDefaults.VerifaliaEmailByEmailCacheKey, email);

            return validationEntry.IsValid(_verifaliaEmailValidatorSettings.AllowRiskyEmails);
        }
