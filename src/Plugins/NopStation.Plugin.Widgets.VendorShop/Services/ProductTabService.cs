...
namespace NopStation.Plugin.Widgets.VendorShop.Services
{
    public class ProductTabService : IProductTabService
    {
        ...
        public async Task<IPagedList<ProductTab>> GetAllProductTabsAsync(int vendorId, List<int> widgetZoneIds = null, bool hasItemsOnly = false,
            int storeId = 0, bool? active = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = _productTabRepository.Table;

            query = query.Where(pt => pt.VendorId == vendorId);
