...
namespace NopStation.Plugin.Widgets.AnywhereSlider.Services
{
    public class SliderService : ISliderService
    {
        ...
        public virtual async Task<IPagedList<Slider>> GetAllSlidersAsync(List<int> widgetZoneIds = null, int storeId = 0,
            bool? active = null, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var cacheKey = _cacheManager.PrepareKeyForDefaultCache(AnywhereSliderCacheDefaults.SlidersAllKey,
                widgetZoneIds, storeId, active, pageIndex, pageSize);
        ...
        {
            var cacheKey = _cacheManager.PrepareKeyForDefaultCache(AnywhereSliderCacheDefaults.SliderItemsBySliderIdKey, sliderId, pageIndex, pageSize);
