...
{
    public class PushNotificationTokenProvider : IPushNotificationTokenProvider
    {
        #region Fields
        ...
        public PushNotificationTokenProvider(CatalogSettings catalogSettings,
        ...
        public IEnumerable<string> GetTokenGroups(PushNotificationTemplate pushNotificationTemplate)
        {
            ...
            {
                case PushNotificationTemplateSystemNames.CustomerRegisteredNotification:
                case PushNotificationTemplateSystemNames.CustomerWelcomeMessage:
                case PushNotificationTemplateSystemNames.CustomerEmailValidationMessage:
                    return new[] { TokenGroupNames.StoreTokens, TokenGroupNames.CustomerTokens };

                case PushNotificationTemplateSystemNames.OrderPlacedVendorNotification:
                case PushNotificationTemplateSystemNames.OrderPlacedAdminNotification:
                case PushNotificationTemplateSystemNames.OrderPaidCustomerNotification:
                case PushNotificationTemplateSystemNames.OrderPaidVendorNotification:
                case PushNotificationTemplateSystemNames.OrderPlacedCustomerNotification:
                case PushNotificationTemplateSystemNames.OrderCompletedCustomerNotification:
                case PushNotificationTemplateSystemNames.OrderCancelledCustomerNotification:
                    return new[] { TokenGroupNames.StoreTokens, TokenGroupNames.OrderTokens, TokenGroupNames.CustomerTokens };

                case PushNotificationTemplateSystemNames.ShipmentSentCustomerNotification:
                case PushNotificationTemplateSystemNames.ShipmentDeliveredCustomerNotification:
                ...
                case PushNotificationTemplateSystemNames.OrderRefundedAdminNotification:
                case PushNotificationTemplateSystemNames.OrderRefundedCustomerNotification:
                ...
                case PushNotificationTemplateSystemNames.NewForumTopicMessage:
                ...
                case PushNotificationTemplateSystemNames.NewForumPostMessage:
                ...
                case PushNotificationTemplateSystemNames.PrivateMessageNotification:
        ...
        public virtual async Task AddCustomerTokensAsync(IList<Token> tokens, Customer customer)
        {
            ...
            tokens.Add(new Token("Customer.FirstName", await _genericAttributeService.GetAttributeAsync<string>(customer, customer.FirstName)));
            tokens.Add(new Token("Customer.LastName", await _genericAttributeService.GetAttributeAsync<string>(customer, customer.LastName)));
            tokens.Add(new Token("Customer.VatNumber", await _genericAttributeService.GetAttributeAsync<string>(customer, customer.VatNumber)));
            tokens.Add(new Token("Customer.VatNumberStatus", ((VatNumberStatus)await _genericAttributeService.GetAttributeAsync<int>(customer, customer.VatNumberStatusId.ToString())).ToString()));

            var customAttributesXml = await _genericAttributeService.GetAttributeAsync<string>(customer, customer.CustomCustomerAttributesXML);
            ...
            var passwordRecoveryUrl = RouteUrlAsync(routeName: "PasswordRecoveryConfirm", routeValues: new { token = await _genericAttributeService.GetAttributeAsync<string>(customer, NopCustomerDefaults.PasswordRecoveryTokenAttribute), email = customer.Email });
            var accountActivationUrl = RouteUrlAsync(routeName: "AccountActivation", routeValues: new { token = await _genericAttributeService.GetAttributeAsync<string>(customer, NopCustomerDefaults.AccountActivationTokenAttribute), email = customer.Email });
            var emailRevalidationUrl = RouteUrlAsync(routeName: "EmailRevalidation", routeValues: new { token = await _genericAttributeService.GetAttributeAsync<string>(customer, NopCustomerDefaults.EmailRevalidationTokenAttribute), email = customer.Email });
        ...
        public virtual async Task AddOrderTokensAsync(IList<Token> tokens, Order order, int languageId, int vendorId = 0)
        {
            ...
            tokens.Add(new Token("Order.ShippingFirstName", (await orderAddressAsync(order))?.FirstName ?? string.Empty));
            tokens.Add(new Token("Order.ShippingLastName", (await orderAddressAsync(order))?.LastName ?? string.Empty));
            tokens.Add(new Token("Order.ShippingPhoneNumber", (await orderAddressAsync(order))?.PhoneNumber ?? string.Empty));
            tokens.Add(new Token("Order.ShippingEmail", (await orderAddressAsync(order))?.Email ?? string.Empty));
            tokens.Add(new Token("Order.ShippingFaxNumber", (await orderAddressAsync(order))?.FaxNumber ?? string.Empty));
            tokens.Add(new Token("Order.ShippingCompany", (await orderAddressAsync(order))?.Company ?? string.Empty));
            tokens.Add(new Token("Order.ShippingAddress1", (await orderAddressAsync(order))?.Address1 ?? string.Empty));
            tokens.Add(new Token("Order.ShippingAddress2", (await orderAddressAsync(order))?.Address2 ?? string.Empty));
            tokens.Add(new Token("Order.ShippingCity", (await orderAddressAsync(order))?.City ?? string.Empty));
            tokens.Add(new Token("Order.ShippingCounty", (await orderAddressAsync(order))?.County ?? string.Empty));
            tokens.Add(new Token("Order.ShippingStateProvince", await _stateProvinceService.GetStateProvinceByAddressAsync(await orderAddressAsync(order)) is StateProvince shippingStateProvince ? await _localizationService.GetLocalizedAsync(shippingStateProvince, x => x.Name) : string.Empty));
            ...
            tokens.Add(new Token("Order.ShippingZipPostalCode", (await orderAddressAsync(order))?.ZipPostalCode ?? string.Empty));
            tokens.Add(new Token("Order.ShippingCountry", await _countryService.GetCountryByAddressAsync(await orderAddressAsync(order)) is Country orderCountry ? await _localizationService.GetLocalizedAsync(orderCountry, x => x.Name) : string.Empty));
            ...
            tokens.Add(new Token("Order.ShippingCustomAttributes", await _addressAttributeFormatter.FormatAttributesAsync((await orderAddressAsync(order))?.CustomAttributes ?? string.Empty), true));
            ...
            var orderUrl = RouteUrlAsync(order.StoreId, "OrderDetails", new { orderId = order.Id });
        ...
        public async Task AddNewsLetterSubscriptionTokensAsync(IList<Token> tokens, NewsLetterSubscription subscription)
        {
            ...
            var activationUrl = RouteUrlAsync(routeName: "NewsletterActivation", routeValues: new { token = subscription.NewsLetterSubscriptionGuid, active = "true" });
            tokens.Add(new Token("NewsLetterSubscription.ActivationUrl", activationUrl, true));

            var deactivationUrl = RouteUrlAsync(routeName: "NewsletterActivation", routeValues: new { token = subscription.NewsLetterSubscriptionGuid, active = "false" });
        ...
        public async Task AddProductTokensAsync(IList<Token> tokens, Product product, int languageId)
        {
            ...
            var productUrl = RouteUrlAsync(routeName: "Product", routeValues: new { SeName = await _urlRecordService.GetSeNameAsync(product) });
        ...
        {
            var forumService = NopInstance.Load<IForumService>();
            var forumUrl = RouteUrlAsync(routeName: "ForumSlug", routeValues: new { id = forum.Id, slug = await forumService.GetForumSeNameAsync(forum) });
        ...
        public async Task AddBackInStockTokensAsync(IList<Token> tokens, BackInStockSubscription subscription)
        {
            ...
            var productUrl = RouteUrlAsync(subscription.StoreId, "Product", new { SeName = await _urlRecordService.GetSeNameAsync(product) });
