    ...
    public class QueuedSmsModelFactory : IQueuedSmsModelFactory
    {
            ...
            QueuedSms queuedSms, bool excludeProperties = false)
        {
            if (queuedSms != null)
            {
                ...
                {
                    model.CustomerName = await _localizationService.GetResourceAsync("Admin.NopStation.VonageSms.QueuedSmss.All");
                }
                else
                {
                    ...
                    {
                        model.CustomerId = 0;
                        model.CustomerName = await _localizationService.GetResourceAsync("Admin.NopStation.VonageSms.QueuedSmss.Unknown");
                    }
