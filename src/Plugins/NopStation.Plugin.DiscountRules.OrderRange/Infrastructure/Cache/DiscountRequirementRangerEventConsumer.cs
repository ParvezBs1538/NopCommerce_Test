...
namespace NopStation.Plugin.DiscountRules.OrderRange.Cache
{
    public class DiscountRequirementRangerEventConsumer : IConsumer<EntityDeletedEvent<DiscountRequirement>>
    {
        ...
        public async Task HandleEventAsync(EntityDeletedEvent<DiscountRequirement> eventMessage)
        {
            ...
            var setting = await _settingService.GetSettingAsync(string.Format(DiscountRequirementDefaults.ConditionValueSettingsKey, discountRequirement.Id));
            if (setting != null)
